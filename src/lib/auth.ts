...
export const getUserWishlist = async (): Promise<WishlistItem[]> => {
  ...
  // Normalize the embedded relation: PostgREST may return a single object
  // or an array depending on the relationship cardinality.
  ...
  return (data || []).map((row: WishlistRow) => ({
    id: row.id,
...
export const redeemLoyaltyGift = async (giftId: string, shippingDetails?: {
  ...
  const requestBody: { gift_id: string; shipping_details?: NonNullable<typeof shippingDetails> } = { gift_id: giftId };
  if (shippingDetails) {
