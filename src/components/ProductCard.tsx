  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    dispatch({
      ...
      payload: {
        ...
        deliveryFrequency: product.isSubscription ? 'Giao hàng mỗi 8 tuần' : '',
      },
    });
    ...
    <div className="bg-white rounded-lg shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden group flex flex-col h-full hover:scale-[1.02] min-h-[400px]">
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-1">
        <div className="mb-2 flex items-center justify-between gap-2">
          <span className="text-xs sm:text-sm text-carehub-teal font-medium bg-green-50 px-2 py-1 rounded truncate">
            {product.category}
          </span>
          {product.brand && product.brand !== 'CareHub' && (
            <span className="text-[10px] sm:text-xs text-gray-500 font-medium bg-gray-100 px-2 py-1 rounded whitespace-nowrap">
              {product.brand}
            </span>
          )}
        </div>
        ...
        <Link to={`/product/${product.id}`}>
          <h3 className="font-semibold text-sm sm:text-base lg:text-lg text-gray-900 mb-3 hover:text-carehub-teal transition-colors line-clamp-2 h-12 sm:h-14 flex items-start leading-tight">
            {product.name}
          </h3>
        </Link>
