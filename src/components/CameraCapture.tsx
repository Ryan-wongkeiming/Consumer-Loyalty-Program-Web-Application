...
const CameraCapture: React.FC<CameraCaptureProps> = ({ onCodeDetected, onClose }) => {
  ...
  useEffect(() => {
    ...
    // Mount/unmount-only: start on mount, clean up the stream on unmount.
    // startCamera/stopCamera are intentionally excluded so the camera is not
    // restarted when unrelated state changes.
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex space-x-3">
            <button
              ...
              className="flex-1 bg-carehub-teal text-white py-3 px-4 rounded-lg font-medium disabled:opacity-50 disabled:cursor-not-allowed hover:bg-carehub-teal-dark transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl"
            >
              {processing ? (
